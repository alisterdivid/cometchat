...
if(file_exists(dirname(dirname(__FILE__)).DIRECTORY_SEPARATOR.'file'.DIRECTORY_SEPARATOR.'settings'.DIRECTORY_SEPARATOR.'server.sett.php')){
	include_once(dirname(dirname(__FILE__)).DIRECTORY_SEPARATOR.'file'.DIRECTORY_SEPARATOR.'settings'.DIRECTORY_SEPARATOR.'server.sett.php');
	define('PHPFOX_VER', '1');
}elseif(file_exists(dirname(dirname(dirname(dirname(dirname(__FILE__))))).DIRECTORY_SEPARATOR.'PF.Base'.DIRECTORY_SEPARATOR.'file'.DIRECTORY_SEPARATOR.'settings'.DIRECTORY_SEPARATOR.'server.sett.php')){
	include_once(dirname(dirname(dirname(dirname(dirname(__FILE__))))).DIRECTORY_SEPARATOR.'PF.Base'.DIRECTORY_SEPARATOR.'file'.DIRECTORY_SEPARATOR.'settings'.DIRECTORY_SEPARATOR.'server.sett.php');
	define('PHPFOX_VER', '0');
}else{
	echo "Please check if CometChat is installed in the correct directory.<br /> The 'cometchat' folder should be placed at <PHPFOX_HOME_DIRECTORY>/cometchat";
 	exit;
}
...
$db_usertable_name = 'full_name';                    // Name containing field in the users or members table
$db_avatartable = ' ';
$db_avatarfield = " ".$table_prefix.$db_usertable.".user_image ";
$db_linkfield = ' '.$table_prefix.$db_usertable.'.user_name ';
...
/* FUNCTIONS */
	...
	function getUserID() {
		...
		if(empty($userid) || $userid == 'null'){
			foreach($_COOKIE as $key => $value) {
				if((strpos($key,'core') == 0 && strpos($key,'user_id') == 8) || (strpos($key,'user_id'))){
					$userid = $value;
				}
			}
		}
		if(isset($_REQUEST['hash_val']) && isset($_REQUEST['cc_social_userid'])){
			$sql = "SELECT menu FROM ".TABLE_PREFIX."module WHERE phrase_var_name = 'cc_hash'";
			$result = sql_query($sql, array(), 1);
			$row = sql_fetch_assoc($result);
			$hash = $row['menu'];
			$from_id = $_REQUEST['cc_social_userid'];
			$hashval = md5($hash.$from_id);
	...
	function getFriendsList($userid,$time) {
		global $hideOffline;
		$offlinecondition = '';
		$sql = ("select DISTINCT ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." userid, ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_NAME." username, ".DB_LINKFIELD." link, ".DB_AVATARFIELD." avatar, cometchat_status.lastactivity lastactivity, cometchat_status.lastseen lastseen, cometchat_status.lastseensetting lastseensetting, cometchat_status.status, cometchat_status.message message, cometchat_status.isdevice, cometchat_status.readreceiptsetting readreceiptsetting from  ".TABLE_PREFIX.DB_USERTABLE." left join cometchat_status on ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." = cometchat_status.userid where  ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." NOT IN (select ".TABLE_PREFIX."user_blocked.block_user_id from ".TABLE_PREFIX."user_blocked where user_id = '".sql_real_escape_string($userid)."' UNION select ".TABLE_PREFIX."user_blocked.user_id from ".TABLE_PREFIX."user_blocked where block_user_id = '".sql_real_escape_string($userid)."') AND ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." IN (select ".TABLE_PREFIX."friend.friend_user_id from ".TABLE_PREFIX."friend where ".TABLE_PREFIX."friend.user_id = '".sql_real_escape_string($userid)."') AND ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." IN (select ".TABLE_PREFIX."friend.user_id from ".TABLE_PREFIX."friend where ".TABLE_PREFIX."friend.friend_user_id = '".sql_real_escape_string($userid)."')  ".DB_AVATARTABLE." order by username asc");
		...
		if ((defined('MEMCACHE') && MEMCACHE <> 0) || DISPLAY_ALL_USERS == 1) {
			...
			$sql = ("select DISTINCT ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." userid, ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_NAME." username,".DB_LINKFIELD." link, ".DB_AVATARFIELD." avatar, cometchat_status.lastactivity lastactivity, cometchat_status.lastseen lastseen, cometchat_status.lastseensetting lastseensetting, cometchat_status.status, cometchat_status.message message, cometchat_status.isdevice, cometchat_status.readreceiptsetting readreceiptsetting from ".TABLE_PREFIX.DB_USERTABLE." left join cometchat_status on ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." = cometchat_status.userid where ((cometchat_status.lastactivity > (".sql_real_escape_string($time)."-".((ONLINE_TIMEOUT)*2).")) OR cometchat_status.isdevice = 1) and  ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." NOT IN (select ".TABLE_PREFIX."user_blocked.block_user_id from ".TABLE_PREFIX."user_blocked where user_id = '".sql_real_escape_string($userid)."' UNION select ".TABLE_PREFIX."user_blocked.user_id from ".TABLE_PREFIX."user_blocked where block_user_id = '".sql_real_escape_string($userid)."') ".DB_AVATARTABLE." ".$offlinecondition." order by username asc");
